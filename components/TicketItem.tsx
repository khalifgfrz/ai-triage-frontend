...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full ${style.bg} px-2.5 py-1 text-xs font-medium ${style.text}`}>
      {status}
    </span>
  );
}
...
export default function TicketItem({ ticket }: Props) {
  ...
  return (
    <Link
      ...
      className="group flex flex-col gap-3 rounded-xl border border-border bg-card p-5 transition-colors hover:border-muted-foreground/30"
    >
