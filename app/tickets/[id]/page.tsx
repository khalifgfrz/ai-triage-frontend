...
export default async function TicketDetail({ params }: Props) {
  ...
  const urgStyle = ticket.urgency && urgencyConfig[ticket.urgency] ? urgencyConfig[ticket.urgency] : null;
    ...
    <main className="py-10">
      ...
      <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex-1">
          ...
          <div className="mt-3 flex items-center gap-2 flex-wrap">
            <span className={`inline-flex items-center rounded-full ${stStyle.bg} px-2.5 py-1 text-xs font-medium ${stStyle.text}`}>{ticket.status}</span>
            {urgStyle ? (
              ...
              <span className="inline-flex items-center rounded-full bg-muted px-2.5 py-1 text-xs font-medium text-muted-foreground">No urgency</span>
            )}
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-border bg-card p-6">
          ...
          <p className="text-sm leading-relaxed text-muted-foreground whitespace-pre-wrap">{ticket.message}</p>
          {ticket.userEmail && (
        ...
        {/* AI Draft Reply Card */}
        <ResolveTicketForm id={params.id} defaultDraft={ticket.aiDraft ?? "No AI draft available yet."} status={ticket.status} />
      </div>
    </main>
  );
}
